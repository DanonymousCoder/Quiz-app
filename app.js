...
let back2 = document.getElementById("back1");
...
let quizArray = questions.map((quiz, index) => {
    ...
    <select name="select" id="select${index}" style="width: 100%; padding: 10px 5px;margin: 5px 0;border: 0;border-bottom: 1px solid #999; outline: none;background: transparent;">
        <option value="" style="color: grey;text-align: center;">-- Select a value--</option>
...
selectS.forEach((select, index) =>{
    select.addEventListener("change", (event) => {
        ...
        if (questions[index].answer === selectValue) {
          ...
          result += wrong;
          document.getElementByIdr("score").innerHTML = `${result}/150`;
        }
      });
});

form3.innerHTML = `${contenx}${quizResult.join("<br>")}<br>${contex}`;
